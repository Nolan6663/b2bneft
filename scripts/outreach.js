...
async function main() {
    ...
    // Массовая рассылка по адресам, которые мы собрали сами (реестр ГИСП + парсер
    // сайтов), — это реклама по сетям электросвязи. ФЗ-38 «О рекламе», ст. 18 ч. 1
    // разрешает её только с предварительного согласия адресата, и доказывать
    // согласие обязан отправитель; ссылка «отписаться» здесь не помогает, потому
    // что закон требует согласия ДО письма, а не возможности отказаться после.
    // Ответственность — ст. 14.3 ч. 1 КоАП, и считается она за каждое письмо.
    // Поэтому отправка выключена по умолчанию и включается осознанно, одной
    // переменной, когда появится законное основание.
    // --dry-run и --test на свой ящик под запрет не подпадают: адресат — мы сами.
    if (SEND && process.env.OUTREACH_ENABLED !== '1') {
        console.error('Массовая рассылка выключена: OUTREACH_ENABLED != 1 в .env.');
        console.error('Это не техническая настройка — см. комментарий выше и docs/LEGAL-COMPLIANCE.md.');
        process.exit(1);
    }
