...
-------------------------------
  ...
  • server.js — GET /api/public/geo-density: агрегат поставщиков по координатам
    (ROUND до 1°, только producer с lat/lng), in-memory кэш 1 час
  • assets/lp-voxel-map.js — высота столбиков = реальная плотность поставщиков
    (буст с falloff вокруг точек агрегата); оранжевые акценты у хотспотов,
    синие — декоративные; фолбэк на псевдорельеф при недоступности API
  • landing.html — подпись «ВЫСОТА СТОЛБИКА — ПЛОТНОСТЬ ПОСТАВЩИКОВ В РЕГИОНЕ»
    показывается ТОЛЬКО при живых данных (не врём); параллакс-наклон сцены за
    курсором (кроме prefers-reduced-motion); пины — ссылки на /zakupki.html
  • scripts/voxel-screenshot.js — MOCK_DENSITY=1 подсовывает тестовую плотность
    (локально нет БД); NO_WEBGL=1 — фолбэк; map.html (Яндекс) не тронут

  Проверка: npm run check; MOCK_DENSITY=1 node scripts/voxel-screenshot.js
  ...
  • routes/companies.js — won_deals в топ-5 поставщиков считал status='Принято',
    который никогда не пишется (реальный — 'Выигран'); счётчик всегда был 0
  • proposals.html — KPI «Выиграно»/«На рассмотрении»/конверсия считали по
    мёртвым статусам ('Победитель', 'На рассмотрении', 'Отклонено') → всегда 0;
    заменены на реальные ('Выигран', 'Ожидает ответа', 'Отклонен'/'Проигран')
  • messages.html — карта статусов КП в контекст-панели и кнопка «Договорённость
    зафиксирована» — на реальные значения БД
  • zakupki.html — убраны FAKE_COMPANIES (выдуманные «ООО …» под blur);
    вместо них честная mono-подпись «Заказчик виден после регистрации»
  • ecosystem.config.js — NODE_ENV=production (в .env на VPS его не было, прод
    работал в development: куки без Secure, dev-CORS, dev-сидинг). После правки
    этого файла на VPS обязательно: pm2 delete neft && pm2 start ecosystem.config.js
    (обычный pm2 restart конфиг НЕ перечитывает). Проверка: /api/health → env

  Проверка: npm run check; curl /api/health
  ...
  AI-провайдер (lib/ai-client.js):
    ...
    • repairLlmJson — вторая ступень того же разбора (02.08.2026). GigaChat
      ломает JSON двумя стабильными способами: живой перевод строки внутри
      строкового значения и лишний пустой литерал вместо запятой между полями
      ("description": "..." затем строка "", затем "checklist"). Проход по
      символам экранирует управляющие символы внутри строк, выбрасывает пустой
      литерал, приклеенный к предыдущему значению, и дописывает пропущенную
      запятую. До этого генерация ТЗ на проде падала 500 на всех запросах.
    • При провале разбора ai-client всегда пишет в лог провайдера, модель,
      finish_reason, расход токенов и первые 400 знаков ответа (раньше это
      было только под AI_DEBUG_RAW, и pm2 logs молчал).
  ...
  ПОСЛЕДНИЕ ОБНОВЛЕНИЯ (03.07.2026 — AI-slop, проход 2: движение и форма)
  ----------------------------------------------------------------
  Проход 1 убрал градиенты; этот убирает шаблонное ДВИЖЕНИЕ и ФОРМУ:
  • Hover-подпрыгивания (translateY(-Npx)) сняты везде: карточки, кнопки,
    kpi, тарифы, каталог. Замена: смена цвета рамки; для карточек каталога/
    заказов — жёсткая офсетная тень 3px 3px 0 (чертёжный язык).
  • Убран глобальный лифт .card:hover/.section:hover из "DESIGN REVAMP"
    секции theme-v2.css (поднимал ВСЕ секции на всех страницах).
  • Мягкие свечения (box-shadow N px N px rgba(оранж/син)) на кнопках →
    жёсткая тень-подложка 0 3px 0 #B54B00 + нажатие на :active
    (login btn-primary, settings save, ai-tz-btn, lp-demo-nav primary).
  • «Парящие» чипы лендинга (@keyframes lp-float) удалены.
  • Пилюли border-radius:999px на бейджах/чипах → 3px прямоугольники
    (hero-бейджи лендинга, kp-чипы, winner-badge, risk-pill, пилюли карты,
    plan-pin тарифов). Прогресс-бары и функц. шкала red→green не тронуты.
  • Оставлено намеренно: blur на модальных оверлеях, skeleton-shimmer,
    blueprint-сетки, lp-cta-primary «печать» (фаска + 4px 4px 0 + press).
  • theme-v2.css ?v=8/9 → v=10 во всех HTML (+settings-page.css v=10).
  ВНИМАНИЕ (урок): массовые замены в HTML через PowerShell — ТОЛЬКО с явным
  UTF-8 чтением/записью ([IO.File]::ReadAllText/WriteAllText + UTF8Encoding
  без BOM), иначе кириллица превращается в кракозябры; npm run check ловит.

  ПОСЛЕДНИЕ ОБНОВЛЕНИЯ (03.07.2026 — фаза B реестра: обогащение + отписка)
  ----------------------------------------------------------------
  Движок приглашений (lib/registry-invites.js) был мёртвым: у всех 4286
  заглушек contact_email='' и пустая продукция (грид ГИСП отдаёт только
  название/ИНН/ОГРН/регион) → 0 писем, matchScore всегда 0. Исправлено:
  • server.js — GET /api/registry-invites/optout: рабочая отписка из письма
    (раньше ссылка вела в 404). HMAC-токен по ИНН, ставит invite_optout=true
    только заглушкам gisp-pp719.
  • scripts/fetch-gisp-enrich.js — обогащение через JSON-RPC каталога
    компаний ГИСП (/company-catalog/rpc — БЕЗ CSRF/HMAC, в отличие от
    бэкенда pp719v2, который требует x-ajax-token+x-csrftoken и отдаёт
    HTML-оболочку на прямые запросы):
      company.list (весь каталог 49k страницами по 1000; фильтр по нашим
      ИНН локально) → email/site/contactPhone/shortName/ОКВЭД;
      company.productsByOGRN → названия продукции.
    ЗАПУСК ТОЛЬКО с ПК с ВЫКЛЮЧЕННЫМ VPN (~1 час, чекпойнт каждые 200,
    повторный запуск продолжает). Итог: scripts/data/registry-gisp-enrich.json
    (4282 контактов: email 204, сайт 2593, телефон 2385; продукция 4286/4286).
  • scripts/import-enrich.js [--dry-run] — заливка в БД: только claimed=false
    source='gisp-pp719', пустым значением ничего не затирает. БД доступна
    только с VPS — запускать там после деплоя.
  ВАЖНО: рассылка станет живой сразу после import-enrich на VPS (204 завода
  с email). Перед тестовыми закупками на проде — REGISTRY_INVITES_ENABLED=0.

  ПОСЛЕДНИЕ ОБНОВЛЕНИЯ (03.07.2026 — фикс: «Итоги недели» наезжали на «Аналитику»)
  ----------------------------------------------------------------
  Глобальное правило table{min-width:680px} (для скролла больших таблиц)
  распирало компактную таблицу виджета за пределы карточки — строки ложились
  поверх соседнего виджета. theme-v2.css: .bottom-widget table{min-width:0}
  + overflow:hidden на .bottom-widget.
  ...
  ПОСЛЕДНИЕ ОБНОВЛЕНИЯ (04.07.2026 — РТИ-производители с fabricators.ru)
  ----------------------------------------------------------------
  Второй источник стабов каталога (первый — ГИСП ПП-719): fabricators.ru,
  категория «резинотехнические изделия». 259 компаний, из них 244 новых,
  15 пересечений с ГИСП (им только дозаполнены контакты). Email у 257!
  Карточки fabricators открытые: JSON-LD отдаёт полный телефон/сайт/email
  («скрытие» только в вёрстке) + ИНН/ОГРН в блоке реквизитов.
  • scripts/fetch-fabricators.js <category-slug> — скрейпер (ПК, VPN не
    мешает, ~5 мин на 263 карточки, чекпойнт/резюм). Другие категории
    fabricators.ru можно снимать тем же скриптом.
  • scripts/import-registry.js --source fabricators — импорт на VPS.
    Новое: сохраняет website/contact_email/phone; если ИНН уже есть как
    стаб ДРУГОГО источника — дозаполняет только пустые поля, source и
    ГИСП-обогащение не трогает.
  • Честные бейджи: rowToCompany отдаёт fromGisp (только gisp-pp719) рядом
    с fromRegistry (любой стаб). «Реестр Минпромторга» — только fromGisp;
    fabricators-стабы: «Из открытых данных» (catalog, company-profile,
    supplier-public, SSR-title /p/:id). Claim CTA — у всех стабов.
  • Инвайт-письма: формулировка про ПП-719 только gisp-стабам, остальным —
    про открытые данные (registry-invites.js читает stub.source).
    Отписка расширена: source <> '' вместо ='gisp-pp719'.
  Геокодинг новых стабов — сам через geocodeExisting() при старте PM2.
  ВАЖНО: после импорта на VPS 257 РТИ-заводов входят в рассылку инвайтов —
  перед тестовыми закупками на проде REGISTRY_INVITES_ENABLED=0.

  ПОСЛЕДНИЕ ОБНОВЛЕНИЯ (04.07.2026 — договор v2 по образцу Севмаша)
  ----------------------------------------------------------------
  Шаблон buildContractPdf дополнен по реальному договору АО ПО Севмаш
  с ИП-поставщиком (изделия по чертежу): гарантия 12 мес + рекламации
  (новый раздел 5), товар новый/не в залоге (1.3), твёрдая цена всё
  включает (2.2), отказ при просрочке >30 дней с возвратом аванса (3.3),
  страна происхождения в спецификации, Приложение 2 (форма акта приёмки),
  Приложение 3 (антикоррупционная оговорка). НДС-строка по системе
  налогообложения поставщика: companies.tax_system (osn/usn/пусто),
  селект в settings (блок Реквизиты). PDF стал 5 страниц.

  ПОСЛЕДНИЕ ОБНОВЛЕНИЯ (04.07.2026 — SLA-метрики поставщика)
  ----------------------------------------------------------------
  lib/company-enrich.js computeProducerStats + avgFirstResponseHours
  (среднее время от публикации закупки до КП, 6 мес, показ от 2 откликов)
  и winRate (% выигранных из решённых, показ от 3 решённых). Пороги —
  чтобы новичкам не рисовать 0% по одному отказу. Отображение:
  supplier-public.html и company-profile.html (KPI-блоки). Схема БД
  не менялась; /api/public/companies/:id отдаёт поля автоматом через stats.
  ...
  ----------------------------------------------------------------
  • index.html — сброс выбора КП при смене закупки в detail-панели
    (раньше «Выбрано: N» залипал от предыдущей закупки)
  • PDF сравнения выгружает выбранные КП (?ids=1,2,3), не все КП закупки;
    server.js — фильтр p.id = ANY($2::int[])
  • proposalStatusBadgeHtml (app.js) + company-profile, partners —
    глиф ⏱ заменён на SVG uiIcon('clock'); уведомление о дедлайне без глифа

  ПОСЛЕДНИЕ ОБНОВЛЕНИЯ (03.07.2026 — фильтры каталога)
  ----------------------------------------------------------------
  • server.js — rowToCompany отдаёт products (реестровые заглушки)
  • catalog.html — CAP_LABELS (ключи lathe/rti/… → русские названия в карточках);
    CHIP_MATCHERS: чипы фильтруют по ключам мощностей + корням слов
    («Токарка» ловит «токарная обработка» из реестра); поиск включает products
  • Причина: чипы искали кириллицу в англ. ключах — фильтр не находил никого
