...
function metaDescription(row) {
    const name = displayName(row);
    const parts = [];
    if (row.specialization) parts.push(String(row.specialization).trim());
    if (row.products) parts.push(String(row.products).split(';').map(s => s.trim()).filter(Boolean).slice(0, 4).join(', '));
...
module.exports = { shortTitle, metaDescription, ssrProfileHtml, displayName, trimAtWord };
