...
const { isSoleTrader, soleTraderShortName } = require('./personal-data');
...
/* Правило и сокращение переехали в lib/personal-data.js: те же ФИО нельзя
   печатать не только в мета-тегах, но и в JSON публичного каталога, и по ним
   нельзя слать письма. Держать регулярку в трёх копиях — значит однажды
   поправить две из них. */
function soleTraderName(row) {
    return soleTraderShortName(row && row.company);
}
