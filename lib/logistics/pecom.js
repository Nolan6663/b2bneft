...
function parsePecomResponse(raw, { doorFrom = true, doorTo = true } = {}) {
    ...
    //
    // Страхование в total НЕ входит — см. комментарий к total ниже.
    ...
    for (const [service, entry] of [['auto', raw.auto], ['avia', raw.avia]]) {
        ...
        quotes.push({
            ...
            service,
            price: {
                // Итог — только перевозка. Страхование сюда не входит намеренно:
                // ПЭК считает его от объявленной нами стоимости, а публичный
                // калькулятор Деловых Линий — по своим правилам и на объявленную
                // стоимость не реагирует вовсе (проверено: и 100 000, и 1 000 000
                // дают ту же страховку). Сложить это в одно число и назвать
                // сравнением было бы враньём — по нему сортируется выдача.
                total: line + pickup + delivery,
                line,
                pickup,
                delivery,
                insurance,
