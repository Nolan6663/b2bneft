...
/* Уточнения, которые обозначают часть того же города, а не соседний
   населённый пункт с тем же названием. Список короткий и таким должен
   остаться: каждое слово здесь означает «этот кандидат — не самостоятельный
   пункт», и ошибка тут молча увезёт груз не туда. Район («р-н») сюда не
   входит никогда. */
const PART_OF_CITY = /^(?:пром\.?\s*зона|промзона|промышленная\s+зона|промрайон)$/i;
...
function normalizeCityName(raw) {
    ...
    // «Санкт-Петербург, Ленинградская обл» — регион после запятой отбрасываем.
    // Так пишут часто, и без этого город не находился вовсе (нашлось отчётом
    // scripts/audit-company-cities.js). Берём часть до запятой: она всегда
    // крупнее и ближе к тому, что человек имел в виду.
    s = s.split(',')[0];
...
function pickCity(rows) {
    ...
    /* Город побеждает свою же промзону.
     *
     * «Зеленоград» и «Зеленоград (промзона)» лежат в справочнике с одним
     * ключом и оба не терминалы — расчёт отвечал «уточните район», и выбрать
     * можно было только промзону: у неё есть уточнение, у города нет. Человек
     * выбирал город из подсказок и не мог его закрепить (замечено на живом
     * расчёте Зеленоград → Химки 18.08).
     *
     * Правило узкое намеренно. Уточнение уточнению рознь: «(Алтайский р-н)» —
     * это другой населённый пункт с тем же именем, и выбирать там должен
     * человек (см. тест про два «Белых Яра»). А «(промзона)» — часть того же
     * города, и когда написано просто «Зеленоград», имеется в виду город.
     * Поэтому смотрим не на наличие уточнения, а на его смысл. */
    const plain = list.filter((r) => !String(r.qualifier || '').trim());
    const rest = list.filter((r) => String(r.qualifier || '').trim());
    if (plain.length === 1 && rest.length && rest.every((r) => PART_OF_CITY.test(r.qualifier))) {
        return { status: 'ok', point: rowToPoint(plain[0]), candidates: [] };
    }
...
async function resolveCity(pool, query) {
    ...
    const { rows } = await pool.query(
        `SELECT id, name, qualifier, pecom_id, pecom_hub, dellin_code, vozovoz_guid, is_hub
    ...
    /* Уточнение района из скобок разводит двойников.
       Выбрав в подсказках «Белый Яр (Алтайский р-н)», человек сохраняет в
       профиль именно эту строку. Ключ поиска скобки отбрасывает — иначе город
       не нашёлся бы вовсе, — но для выбора между двумя «Белыми Ярами» они как
       раз и нужны. Без этого подсказки не помогали бы там, где нужнее всего. */
    const wanted = extractQualifier(query).toLowerCase();
    if (wanted) {
        const narrowed = rows.filter((r) => String(r.qualifier || '').toLowerCase() === wanted);
        if (narrowed.length) return pickCity(narrowed);
    }
...
/**
 * Дописывает городу код перевозчика, если его ещё нет.
 *
 * Справочник ПЭК отдаётся целиком одним файлом и лежит у нас таблицей. У
 * Деловых Линий и Возовоза такого файла нет — код добывается их же поиском по
 * одному городу. Поэтому лениво: спросили один раз при первом расчёте по этому
 * городу и запомнили.
 *
 * lookup передаётся снаружи, чтобы гео-слой не зависел от модулей перевозчиков
 * и проверялся без сети. Не нашли — не беда: этот перевозчик просто не
 * посчитает маршрут, а остальные посчитают.
 *
 * Колонка подставляется в SQL, а не параметром: параметром имя колонки в
 * PostgreSQL не передать. Значения списком ниже закрыты, снаружи сюда попасть
 * нечему.
 */
const CARRIER_CODE_COLUMNS = { dellin: 'dellin_code', vozovoz: 'vozovoz_guid' };

async function fillCarrierCode(pool, point, carrier, lookup) {
    const column = CARRIER_CODE_COLUMNS[carrier];
    if (!column) throw new Error(`Неизвестный перевозчик: ${carrier}`);
    if (!point || !point.id || (point.codes && point.codes[carrier])) return point;

    let code = null;
    try {
        code = await lookup(point.name);
    } catch {
        return point;   // поиск недоступен — считаем без этого перевозчика
    }
    if (!code) return point;

    await pool.query(
        `UPDATE logistics_cities SET ${column} = $1, updated_at = NOW() WHERE id = $2`,
        [code, point.id]
    );
    point.codes[carrier] = code;
    return point;
}

const fillDellinCode = (pool, point, lookup) => fillCarrierCode(pool, point, 'dellin', lookup);
const fillVozovozGuid = (pool, point, lookup) => fillCarrierCode(pool, point, 'vozovoz', lookup);
