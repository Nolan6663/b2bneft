 ...
 * Возвращает { quotes, failed, silent, fromCache }.
 *
 * failed — кто ответил ошибкой или не ответил вовсе.
 * silent — кто вернул пустоту: маршрута нет в его справочнике, кода города не
 * нашлось, ключа к API нет. Раньше такие просто исчезали из списка, и человек
 * видел одного перевозчика вместо трёх, не понимая, отказались остальные или
 * их вообще не спросили. Замечено на живом расчёте Зеленоград → Химки 18.08:
 * ответил только ПЭК, а в failed было пусто.
 */
async function quoteAll(pool, params, options = {}) {
    ...
    settled.forEach((result, i) => {
        ...
        if (result.status === 'fulfilled') {
            ...
            if (!result.value.quotes.length) silent.push(carrier.CARRIER_NAME);
    ...
    return { quotes: sortQuotes(quotes), failed, silent, fromCache };
}
