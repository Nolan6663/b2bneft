...
function parseDellinResponse(raw, { doorFrom = true, doorTo = true } = {}) {
    ...
    for (const [service, block] of services) {
        ...
        quotes.push({
            ...
            service,
            price: {
                total: lineTotal + pickup + delivery + insurance,
                line: lineTotal,
                pickup,
                delivery,
                insurance,
...
async function quote({ from, to, places, insurance = 0, doorFrom = true, doorTo = true }) {
    ...
    const res = await fetch(CALC_URL, {
        ...
        body: JSON.stringify({
            ...
            // Страхуем на цену предложения: у них declaredPrice в рублях.
            declaredPrice: Math.round(Number(insurance) || 0) || undefined,
        }),
        signal: AbortSignal.timeout(TIMEOUT_MS),
    });
