...
function buildQuotesWorkbook(data) {
    ...
    if ((data.failed && data.failed.length) || (data.silent && data.silent.length)) {
        ws.addRow([]);
        if (data.failed && data.failed.length) ws.addRow([`Не ответили на запрос: ${data.failed.join(', ')}`]);
        if (data.silent && data.silent.length) {
            ws.addRow([`Не считают этот маршрут: ${data.silent.join(', ')} — пункта нет в их справочнике`]);
        }
    }
