 ...
 * Gemini (GEMINI_API_KEY) остался только у классификатора поисковых запросов
 * в админском SEO-разделе: умный поиск по каталогу переехал сюда 19.08.2026.
 */

/* Провайдера определяем только по явному указанию.
 *
 *  Раньше последней строкой стояло `return 'openai'`, а базовым адресом для этой
 *  ветки — api.deepseek.com. То есть пустой или опечатанный AI_TZ_PROVIDER молча
 *  уводил тексты закупок заказчиков в Китай, притом что политика обработки
 *  персональных данных (privacy.html, раздел 5) называет единственным получателем
 *  GigaChat. Это расхождение политики с фактом плюс трансграничная передача,
 *  о которой субъект не предупреждён (152-ФЗ, ст. 12).
 *
 *  Незаданный провайдер теперь означает «ИИ не настроен»: генерация ТЗ вежливо
 *  откажет, и это несравнимо дешевле необъявленной передачи данных за рубеж.
 *  Меняя провайдера — правьте и раздел 5 политики, иначе снова разъедется. */
...
/** Базовый адрес для OpenAI-совместимых провайдеров — только по имени, которое
 *  назвали в .env. Ничего не угадываем. */
...
function getTzAiConfig() {
    ...
    if (!provider) {
        // AI_TZ_PROVIDER не задан — считаем, что ИИ выключен (см. getProvider).
        return { provider: '', apiKey, model: '', configured: false };
    }
    ...
    // Незнакомое имя провайдера без явного адреса — тоже «не настроено»:
    // угадывать, куда уедет текст закупки, нельзя.
    if (!baseUrl) return { provider, apiKey, model: '', configured: false };
...
// GigaChat стабильно ломает JSON двумя способами, и оба видны в проде на
// генерации ТЗ: живой перевод строки внутри строкового значения (по стандарту
// там обязан быть \n) и лишний пустой литерал вместо запятой между полями:
//     "description": "...текст..."
//     "",
//     "checklist": [...]
// Чиним обе поломки одним проходом по символам, чтобы не гадать регулярками:
// внутри строки экранируем управляющие символы, снаружи выбрасываем пустой
// литерал, приклеенный к предыдущему значению, и дописываем пропущенную запятую.
function repairLlmJson(text) {
    ...
    for (let i = 0; i < src.length; i++) {
        ...
        if (ch === '"') {
            // пустой литерал сразу после закрытой строки — мусор вместо запятой
            if (lastSignificant === '"' && src[i + 1] === '"') {
                i++;
                continue;
            }
            // значение кончилось, началось следующее поле, а запятой нет
...
function parseJsonFromLlm(text) {
    ...
    try {
        ...
        if (block) {
            ...
            } catch { /* ниже пробуем починить */ }
        }
        try {
            return JSON.parse(repairLlmJson(block || cleaned));
        } catch { /* отдаём исходную ошибку разбора */ }
        throw e;
    }
}
