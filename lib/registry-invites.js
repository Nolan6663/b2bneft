...
function createRegistryInviter({ pool, sendEmail, appUrl, jwtSecret }) {
    ...
    function inviteHtml(order, stub) {
        // UTM до #: hash сохраняется при переходе, а Метрика увидит источник
        const claimUrl = `${appUrl}/login.html?utm_source=registry-invite&utm_medium=email&utm_campaign=order-invite#register?claim=${encodeURIComponent(stub.inn)}&company=${encodeURIComponent(stub.company)}`;
