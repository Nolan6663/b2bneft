...
function emptyState(entity) {
    /* Кнопка подписки настоящая, а не ссылка на регистрацию: до появления
       lib/order-subscriptions здесь стояло обещание, которого платформа не
       выполняла, — тот же §6.6 запрещает и это. Данные темы подставляет
       сервер, обработчик живёт в шаблоне страницы. */
    const kindAttr = entity.kind === 'service'
        ? `data-service-id="${Number(entity.id)}"`
        : `data-product-id="${Number(entity.id)}"`;
    ...
    <ul class="zr-actions">
      <li><button type="button" class="zr-subscribe" ${kindAttr} data-topic="${esc(entity.name)}">Подписаться на новые заказы</button>
        — уведомление придёт, как только появится подходящая заявка.</li>
