...
// ИП из выборки исключены: их наименование — ФИО физлица, см. lib/personal-data.js.
// Сама отправка включается только при OUTREACH_ENABLED=1 (scripts/outreach.js).
        ...
        const { rows } = await pool.query(
            `SELECT c.id, c.company, c.inn, c.city, c.specialization, c.products, c.contact_email
             ...
             WHERE c.role = 'producer' AND c.claimed = false AND c.invite_optout = false
               AND ${excludeUnclaimedSoleTradersSql('c')}
               AND c.contact_email <> ''
