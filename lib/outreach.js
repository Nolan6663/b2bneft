...
- Тема письма: конкретная, до 60 знаков, без слов «реклама», «скидка», «бесплатно», без восклицаний и капса.
...
function buildUserPrompt(stub) {
    return [
        `Завод: ${stub.company}`,
        stub.city ? `Город: ${stub.city}` : null,
...
function fallbackLetter(stub) {
    return {
        subject: `Прямые заказы для «${String(stub.company).slice(0, 40)}» на ТехЗаказ`,
        paragraphs: [
...
module.exports = { createOutreach, buildUserPrompt, fallbackLetter, OUTREACH_SYSTEM_PROMPT };
