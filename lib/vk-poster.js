...
function createVkPoster({ pool, appUrl, fetchImpl }) {
    ...
    async function postOne(row) {
        ...
        if (!orderRow) {
            await pool.query("UPDATE vk_posts SET status = 'skipped', last_error = 'закупка удалена' WHERE id = $1", [row.id]);
            return;
        }
        try {
    ...
    return { enqueue, tick, start, buildMessage, isEnabled, isTestCompany };
}
