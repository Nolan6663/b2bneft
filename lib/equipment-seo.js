 ...
 *  Значение по умолчанию; фактическое — в настройке threshold.equipment.supply.
 *  Ключ отдельный от гео: «Краулинговый бюджет» §4.1 порога для страниц
 *  оборудования не задаёт, а приравнивать их к регионам никто не просил. */
const MIN_INDEXABLE = 5;

function minIndexable() {
    return getThreshold('threshold.equipment.supply', MIN_INDEXABLE);
}
...
function buildOperationTitle(op, total) {
    if (hasOwnContent(op) && total < minIndexable()) {
...
function buildOperationDescription(op, total) {
    const head = hasOwnContent(op) && total < minIndexable()
...
function isOperationIndexable(op, total) {
    return total >= minIndexable() || hasOwnContent(op);
}
