 ...
 *  Значение по умолчанию; фактическое берётся из настройки threshold.geo.supply,
 *  чтобы порог правился без релиза (ответ маркетинга на вопрос 1). */
const MIN_INDEXABLE = 5;

function minIndexable() {
    return getThreshold('threshold.geo.supply', MIN_INDEXABLE);
}
...
/** Звать ли робота на страницу региона. Общий предикат для мета-тега и карты
 *  сайта: если спрашивать порог в двух местах по отдельности, они разойдутся при
 *  первой же правке настройки, и карта позовёт робота туда, где стоит noindex. */
function isRegionIndexable(total) {
    return total >= minIndexable();
}

function buildRegionRobots(total) {
    return isRegionIndexable(total) ? 'index, follow' : 'noindex, follow';
}
