...
function buildRegionTitle(region, total) {
    return `Производители ${region.where}: ${total} ${plural(total, 'предприятие', 'предприятия', 'предприятий')} — ТехЗаказ`;
}

function buildRegionDescription(region, stats) {
    const total = stats.total || 0;
    const top = (stats.categories || []).filter(([name]) => name !== 'Прочее').slice(0, 3).map(([name]) => name.toLowerCase());
