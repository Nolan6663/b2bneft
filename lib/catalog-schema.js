...
async function initCatalog(pool) {
    await pool.query(`
        ...
        /* ── Заказ ↔ справочник ───────────────────────────────────────────
           ТЗ §4.1 связывает Order с услугами и изделиями, и без этих связей
           тематический хаб заказов приходится собирать поиском подстроки по
           заголовку. Так «вал» находит «вальцовку», а «резка» — «нарезку
           резьбы»: страница наполняется чужими заявками, и исполнитель,
           пришедший из поиска, видит не то, что ему обещали.

           confirmation здесь тот же, что у связей компаний: 'auto' — разбор
           текста заявки, 'claimed' — заказчик выбрал сам в мастере. Ручной
           выбор сильнее: ТЗ §6.8 требует «определение изделия и услуг с
           возможностью ручного исправления», и исправление обязано побеждать. */
        ...
        /* ── Кейсы исполнителей ───────────────────────────────────────────
           ТЗ §9. Кейс — это структурированное подтверждение выполненной работы,
           и в нём две разные части. Текстовая («задача, сложность, решение,
           результат») делает страницу полезной человеку. Числовая (габариты,
           масса, точность, шероховатость) подтверждает, что предприятие такое
           действительно умеет, — из неё же вырастают производственные
           возможности, которых ТЗ §4.2 ждёт в структурированном виде.

           Конфиденциальность отдельным полем: назвать заказчика можно только с
           его разрешения (ТЗ §9.2). По умолчанию обезличено — исполнитель
           должен сознательно подтвердить, что имя раскрывать разрешено, а не
           забыть снять галочку. */
        CREATE TABLE IF NOT EXISTS cases (
            ...
            customer_named  BOOLEAN     NOT NULL DEFAULT false,
        ...
        /* Услуг у кейса может быть несколько: вал точат, потом шлифуют, потом
           закаляют, и каждая из этих страниц вправе показать его у себя. */
        ...
        /* История модерации. ТЗ §9.4 требует, чтобы автор видел, что происходило
           с его кейсом и почему он вернулся на доработку. Без журнала
           единственным ответом было бы «отклонён», после чего исполнитель
        ...
        /* ── Подписка исполнителя на заказы ───────────────────────────────
           ТЗ §6.6 требует подписку на первом экране хаба заказов, и пустое
           состояние хаба её уже обещает. Обещание без механизма — это ровно
           то, что тот же §6.6 запрещает: «текст не должен обещать
           несуществующие заявки».

           Подписка привязана к сущности справочника, а не к строке поиска:
           «заказы на токарную обработку» — это устойчивая тема, которую мы
           умеем сопоставлять с заявкой (lib/order-linking), а произвольная
           фраза потребовала бы полнотекстового поиска на каждую новую заявку. */
        CREATE TABLE IF NOT EXISTS order_subscriptions (
            ...
            -- Одна подписка на тему: повторное нажатие кнопки не должно
            -- превращаться во второе письмо о том же заказе.
            UNIQUE (company, service_id, product_id)
        );
    ...
    /* Синонимы и альтернативные названия — требование ТЗ §13.1.
       Отдельной колонкой, а не таблицей: это короткий список строк без
       собственных свойств, и join ради него был бы дороже пользы.

       Практический смысл — в сопоставлении со справочником (lib/catalog-match):
       «токарка» и «точение» в профиле завода означают токарную обработку, но
       из названия сущности это не выводится. Синонимы дают редактору способ
       улучшить разбор текстов, не трогая код. */
    await pool.query(`
        ALTER TABLE services ADD COLUMN IF NOT EXISTS synonyms JSONB NOT NULL DEFAULT '[]'::jsonb;
        ALTER TABLE products ADD COLUMN IF NOT EXISTS synonyms JSONB NOT NULL DEFAULT '[]'::jsonb;

        /* Поля заявки из ТЗ §6.8. Тип производства и материал заметно меняют
           круг подходящих исполнителей: единичный заказ и серия на 10 000 штук
           делаются на разном оборудовании, и заводу важно понять это из
           карточки, а не после переписки. */
        ALTER TABLE orders ADD COLUMN IF NOT EXISTS production_type TEXT NOT NULL DEFAULT '';
        ALTER TABLE orders ADD COLUMN IF NOT EXISTS material        TEXT NOT NULL DEFAULT '';

        /* Падежные формы названия — для заголовков, где именительный читается
           безграмотно: «Заказы на токарную обработку», «Производители валов».
           Склонять кодом нельзя (у «валы» родительный «валов», у «литьё под
           давлением» меняется только первое слово), а обходиться двоеточием
           везде — значит писать «Производители: Валы» там, где маркетинг сам
           называет страницу «Производители валов».

           Поля необязательные: пустое значение возвращает конструкцию с
           двоеточием, то есть страница остаётся грамотной и без них. */
        ALTER TABLE services ADD COLUMN IF NOT EXISTS accusative TEXT NOT NULL DEFAULT '';
        ALTER TABLE services ADD COLUMN IF NOT EXISTS genitive   TEXT NOT NULL DEFAULT '';
        ALTER TABLE products ADD COLUMN IF NOT EXISTS accusative TEXT NOT NULL DEFAULT '';
        ALTER TABLE products ADD COLUMN IF NOT EXISTS genitive   TEXT NOT NULL DEFAULT '';
    `);
    ...
    await pool.query(`
        ...
        CREATE INDEX IF NOT EXISTS idx_case_events      ON case_events (case_id, created_at DESC);
    `);
...
/** Значение meta robots для посадочной страницы.
 *
 *  follow остаётся всегда, даже у неиндексируемых: страница не должна попадать
 *  в индекс, но обойти по её ссылкам робот обязан — иначе связанные разделы
 *  окажутся недостижимыми. Единственное исключение — merged и archived: там
 *  ходить уже некуда, ответ отдаёт редирект или 410. */
function robotsFor(landing) {
    return isIndexable(landing) ? 'index, follow' : 'noindex, follow';
}
