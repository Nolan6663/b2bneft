...
function _obSteps(role) {
  ...
  return [
    ...
    /* «Более 100 верифицированных» — цифра из раннего макета: в каталоге больше
       четырёх тысяч предприятий из государственного реестра, и проверены
       платформой далеко не все. Пишем то, что есть на самом деле. */
    { id: 'catalog',  label: 'Изучите каталог поставщиков', desc: 'Более 4500 предприятий из реестра Минпромторга',        href: 'catalog.html' },
...
function _showObWelcome(role) {
  ...
  const subtitle = isProducer
    ...
    /* «нефтесервисной отрасли» убрано: площадка шире одной отрасли, а
       заказчику важно, что отвечать будут заводы, а не перекупщики. */
    : 'Размещайте закупки и получайте КП от проверенных заводов';
