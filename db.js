...
async function initDb() {
    ...
    /* Настоящий город предприятия.
     *
     * В колонке city у 3240 карточек из 4500 стоит не город, а регион:
     * реестр ГИСП пишет туда «Удмуртская Республика», и по этой строке
     * группируются региональные страницы — переписать её значит сломать их.
     * Поэтому город живёт отдельно, а старая колонка остаётся регионом.
     * Название неудачное с обеих сторон, но менять смысл существующей колонки
     * на живой базе дороже, чем завести соседнюю с честным комментарием.
     *
     * Заполняется скриптом scripts/backfill-cities.js по ИНН через DaData.
     */
    await pool.query('ALTER TABLE companies ADD COLUMN IF NOT EXISTS town TEXT');
