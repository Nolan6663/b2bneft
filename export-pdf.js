...
// Формулировка НДС по системе налогообложения поставщика
function vatClause(supplier) {
  const tax = supplier && supplier.taxSystem;
  if (tax === 'usn') return 'НДС не облагается на основании глав 26.2 Налогового кодекса РФ (Поставщик применяет упрощённую систему налогообложения).';
  if (tax === 'osn') return 'в том числе НДС 20%.';
  return 'НДС — в соответствии с применяемой Поставщиком системой налогообложения.';
}
...
function buildContractPdf(data, res) {
  ...
  pipePdf(res, `dogovor-${docNo}.pdf`, (doc) => {
    ...
    p('1.3. Поставляемая Продукция должна быть новой, не бывшей в употреблении и не восстановленной, не обременена правами третьих лиц, не находится под арестом и не является предметом залога.');

    h('2. ЦЕНА ДОГОВОРА И ПОРЯДОК ОПЛАТЫ');
    p(`2.1. Цена Договора составляет ${fmtNum(price)} руб. (${rublesInWords(price)}), ${vatClause(supplier)}`);
    p('2.2. Цена Договора является твёрдой, изменению в одностороннем порядке не подлежит и включает стоимость Продукции, все налоги и сборы, стоимость тары (упаковки), маркировки и погрузочных работ, если иное не указано в Спецификации.');
    p('2.3. Оплата производится безналичным перечислением на расчётный счёт Поставщика, указанный в разделе 9 настоящего Договора.');
    p(clause.replace('2.3.', '2.4.'));
    ...
    p('3.3. При нарушении Поставщиком срока поставки более чем на 30 (тридцать) календарных дней Покупатель вправе отказаться от исполнения настоящего Договора в одностороннем внесудебном порядке, письменно уведомив Поставщика. Уплаченный аванс в этом случае подлежит возврату в течение 10 (десяти) банковских дней с даты уведомления.');

    h('4. КАЧЕСТВО И ПРИЁМКА');
    p('4.1. Качество Продукции должно соответствовать Спецификации, чертежам и техническому заданию Покупателя, а также применимым ГОСТ/ТУ, указанным в Спецификации. Вместе с Продукцией передаются документы, подтверждающие качество (паспорт, сертификат либо иной документ, предусмотренный для данного вида Продукции).');
    p('4.2. Приёмка Продукции по количеству и качеству производится Покупателем в течение 10 (десяти) рабочих дней с даты получения. О выявленных недостатках Покупатель уведомляет Поставщика письменно в указанный срок. По результатам приёмки Стороны подписывают Акт приёмки (Приложение № 2).');
    p('4.3. При обнаружении недостатков Поставщик обязан за свой счёт устранить их либо заменить Продукцию в течение 20 (двадцати) календарных дней с даты получения уведомления, если иной срок не согласован Сторонами.');

    h('5. ГАРАНТИЙНЫЕ ОБЯЗАТЕЛЬСТВА');
    p('5.1. Гарантийный срок на Продукцию составляет 12 (двенадцать) месяцев с даты приёмки Продукции Покупателем, если иной срок не указан в Спецификации или документации изготовителя.');
    p('5.2. Недостатки, выявленные в течение гарантийного срока, Поставщик устраняет за свой счёт (включая замену Продукции и транспортные расходы) в течение 20 (двадцати) календарных дней с даты получения претензии Покупателя, если иной срок не согласован Сторонами.');
    p('5.3. Если Продукция не могла использоваться по назначению из-за выявленных недостатков, гарантийный срок продлевается на время, в течение которого использование было невозможно.');

    h('6. ОТВЕТСТВЕННОСТЬ СТОРОН');
    p('6.1. За нарушение сроков поставки Покупатель вправе требовать уплаты неустойки в размере 0,1% от стоимости непоставленной в срок Продукции за каждый день просрочки, но не более 10% от Цены Договора.');
    p('6.2. За нарушение сроков оплаты Поставщик вправе требовать уплаты неустойки в размере 0,1% от неоплаченной суммы за каждый день просрочки, но не более 10% от Цены Договора.');
    p('6.3. Во всём ином Стороны несут ответственность в соответствии с законодательством Российской Федерации.');

    h('7. РАЗРЕШЕНИЕ СПОРОВ');
    p('7.1. Споры разрешаются путём переговоров. Претензионный порядок обязателен: срок ответа на претензию — 30 (тридцать) календарных дней с даты получения.');
    p('7.2. При недостижении согласия спор передаётся в арбитражный суд по месту нахождения истца.');

    h('8. ПРОЧИЕ УСЛОВИЯ');
    p('8.1. Договор вступает в силу с даты подписания обеими Сторонами и действует до полного исполнения обязательств.');
    p('8.2. Договор составлен в двух экземплярах, имеющих равную юридическую силу, по одному для каждой из Сторон. Стороны признают юридическую силу документов, переданных по электронной почте (сканированных копий), до момента обмена оригиналами.');
    p('8.3. Стороны соблюдают требования антикоррупционного законодательства Российской Федерации в соответствии с Приложением № 3 к настоящему Договору.');
    p('8.4. Настоящий Договор сформирован на основании данных сделки платформы ТехЗаказ (texzakaz.ru).');
    ...
    // ── Приложение № 2: Акт приёмки (форма) ──
    doc.addPage();
    doc.font('TZ-Bold').fontSize(11).fillColor(TZ_INK)
      .text(`ПРИЛОЖЕНИЕ № 2 к Договору поставки № ${docNo} от ${fmtDate(new Date())}`, L, 50, { width: W_TEXT, align: 'center' });
    doc.moveDown(0.3).fontSize(12).text('АКТ ПРИЁМКИ ПРОДУКЦИИ (форма)', L, undefined, { width: W_TEXT, align: 'center' });
    doc.moveDown(1);
    p(`Настоящим подтверждается, что в соответствии с Договором поставки № ${docNo} Покупателем принята следующая Продукция:`);
    doc.moveDown(0.5);
    let ay = doc.y;
    const actCols = [[30, '№'], [305, 'Наименование Продукции'], [90, 'Кол-во, шт'], [90, 'Примечание']];
    let ax = L;
    for (const [w, label] of actCols) {
      doc.rect(ax, ay, w, 20).lineWidth(0.6).strokeColor(TZ_INK).stroke();
      doc.font('TZ-Bold').fontSize(7.5).fillColor(TZ_INK).text(label, ax + 4, ay + 6, { width: w - 8, lineBreak: false });
      ax += w;
    }
    for (let row = 0; row < 3; row++) {
      ay += 20; ax = L;
      for (const [w] of actCols) { doc.rect(ax, ay, w, 20).stroke(); ax += w; }
    }
    doc.y = ay + 30;
    p('Продукция принята по количеству и качеству, претензий не имеется / имеются следующие замечания: ______________________________________________.');
    doc.moveDown(1.5);
    doc.font('TZ').fontSize(8).fillColor(TZ_INK)
      .text('От Покупателя: ________________ / ________________   «____» ____________ 20___ г.', L, undefined, { width: W_TEXT });

    // ── Приложение № 3: Антикоррупционная оговорка ──
    doc.addPage();
    doc.font('TZ-Bold').fontSize(11).fillColor(TZ_INK)
      .text(`ПРИЛОЖЕНИЕ № 3 к Договору поставки № ${docNo} от ${fmtDate(new Date())}`, L, 50, { width: W_TEXT, align: 'center' });
    doc.moveDown(0.3).fontSize(12).text('АНТИКОРРУПЦИОННАЯ ОГОВОРКА', L, undefined, { width: W_TEXT, align: 'center' });
    doc.moveDown(1);
    p('1. При исполнении обязательств по Договору Стороны и их работники обязуются соблюдать требования антикоррупционного законодательства Российской Федерации.');
    p('2. Стороны, их представители и работники не выплачивают, не предлагают выплатить и не разрешают выплату каких-либо денежных средств или ценностей, прямо или косвенно, любым лицам для оказания влияния на их действия или решения с целью получения неправомерных преимуществ или достижения иных неправомерных целей.');
    p('3. К коррупционным нарушениям для целей Договора относятся, в том числе: дача/получение взятки, коммерческий подкуп, предоставление или получение выгоды в виде денег, ценностей, имущества, услуг или имущественных прав для оказания влияния на действия или решения представителей Сторон.');
    p('4. При возникновении у Стороны обоснованного предположения о произошедшем или возможном коррупционном нарушении она письменно уведомляет другую Сторону с указанием фактов. Сторона, получившая уведомление, обязана рассмотреть его и сообщить о результатах в течение 10 (десяти) рабочих дней.');
    p('5. Нарушение настоящей оговорки является существенным нарушением Договора и даёт добросовестной Стороне право расторгнуть Договор в одностороннем порядке с возмещением причинённых таким нарушением убытков.');
    doc.moveDown(1.5);
    const acY = doc.y;
    doc.font('TZ').fontSize(8).fillColor(TZ_INK)
      .text('ПОКУПАТЕЛЬ: ______________ М.П.', L, acY, { width: colW });
    doc.text('ПОСТАВЩИК: ______________ М.П.', L + colW + 20, acY, { width: colW });
  }, { docNo });
}
