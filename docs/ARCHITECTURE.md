...
Пуш в `main` сначала проходит job `check` (npm ci → `npm run check` → pdf-smoke)
и только потом деплоится. Битый синтаксис/HTML до прода не доедет; логику
CI не проверяет — вручную прогонять флоу после значимых изменений.
...
- Тесты: статические + smoke + Playwright e2e; юнитов на роуты нет.
