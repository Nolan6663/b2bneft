...
| `lib/logistics/quote-doc.js` | общие куски документов с расчётом доставки: подписи, состав цены, оговорка. Один источник для PDF, Excel и экрана — иначе документ и страница разойдутся |
| `lib/logistics/quote-xlsx.js` | тот же расчёт книгой Excel (цены — числа, чтобы СУММ в чужой таблице работал) |
| `export-pdf.js` | все PDF: заявки/КП/сравнение КП/договор+спецификация/расчёт доставки. Кириллица ТОЛЬКО через встроенный JetBrains Mono (`assets/fonts/pdf/`) — стандартная Helvetica её ломает. Рамка+title-block на каждой странице (`drawTitleBlocks`) |
...
7. Сжатие ответов живёт в приложении (`compression` в `server.js`), а НЕ в nginx — у nginx на VPS gzip выключен, и до 15.08.2026 наружу всё уходило несжатым. Если однажды включите gzip в nginx, уберите одно из двух: двойное сжатие бессмысленно.
8. Один AmneziaWG-пир = одно устройство. Тот же конфиг со второго устройства — сервер флип-флопит endpoint, трафик молча умирает.
9. Файлы открываются прямой ссылкой (`window.open`, `<a href>`, `window.location`) — это **не** `apiFetch`: ни повтора запроса, ни обновления токена, ни тоста там нет. Ошибку такому клиенту отдавать через `sendHttpError`, иначе человек получит на экран голый JSON и прочитает это как поломку сайта.
