...
test('мехобработка не воюет со страницей закупок за «металлообработку»', () => {
    /* Кластер на 233 000 показов оказался тремя разными людьми: завод, ищущий
       заказы (84 000), заказчик, ищущий исполнителя (97 000), и собственно
       услуга (70 000). Услугу по чертежу забирает /zakupki/metall, здесь —
       узкие доводочные операции. Две наши страницы под один запрос это не два
       шанса, а размытый сигнал. */
    const MEH = operationBySlug('mehobrabotka');
    const html = buildOperationContent(MEH);
    for (const word of ['Расточные', 'Шлифовка', 'сверление', 'Доводочные']) {
        assert.ok(html.includes(word), `в разборе нет раздела про «${word}»`);
    }
    assert.match(html, /закупок «Металлообработка по чертежу»/, 'нет отсылки к странице, которая забирает общий запрос');
    assert.ok(!/<h1|заказ на металлообработку целиком/i.test(html));

    // Заголовки двух страниц должны читаться как разные ответы, а не как дубли.
    assert.match(buildOperationTitle(MEH, 8), /Механическая обработка/);
    assert.ok(!/Металлообработка по чертежу/.test(buildOperationTitle(MEH, 8)));
});
