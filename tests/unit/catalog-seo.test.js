...
test('заголовок склоняет число: «4531 предприятие», а не «предприятий»', () => {
    // Границу слова здесь пишем явным «дальше не буква»: \b в JS считает
    // кириллицу небуквенной и на «предприятие |» границы не находит вовсе.
    assert.match(buildTitle(1, 4531), /4531 предприятие(?![а-яё])/);
    assert.match(buildTitle(1, 4532), /4532 предприятия(?![а-яё])/);
    assert.match(buildTitle(1, 4535), /4535 предприятий(?![а-яё])/);
});
